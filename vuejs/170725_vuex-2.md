...
이다. 또한, 다음 챕터에 나올 Actions 와의 차이점은

- Mutations 는 동기적 로직을 정의
- Actions 는 비동기적 로직을 정의

가 있겠다. Mutations 의 성격상 안에 정의한 로직들이 순차적으로 일어나야
...
궁금하면 `this.$store` 을 콘솔로 찍어 내용을 확인해보자.
...
## 마무리
지난 글의 state 에 이어서, 이번 글에서는 변경된 state 값을 받아오는 Getters 와
